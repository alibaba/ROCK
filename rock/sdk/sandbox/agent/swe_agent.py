...
class SweAgent(RockAgent):
    """SWE-agent implementation for automated PR fixing.

    Extends RockAgent to provide SWE-agent specific functionality:
    - SWE-agent installation via git clone and pip install
    - Dynamic YAML config generation for sweagent CLI
    - Integration with project_path as repository context
    """
    ...
    async def create_agent_run_cmd(self, prompt: str) -> str:
        ...
        return (
            f"{self.rt_env.bin_dir}/sweagent run "
