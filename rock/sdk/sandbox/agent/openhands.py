...
This code is composed with following parts:
    ...
    2. Upload launch entry code which is modified from [Openhands/benchmarks/swebench/run_infer.py](https://github.com/OpenHands/benchmarks/blob/main/benchmarks/swebench/run_infer.py)
        into Sandbox
...
class OpenhandsConfig(AgentConfig):
    ...
    lan_python_v12_install_cmd: str = "mkdir -p /openhands && cd /openhands && wget -q -O cpython-3.12.12.tar.gz http://tisplus-console.oss-cn-hangzhou.aliyuncs.com/chatos/chatflow/cpython31212.tar.gz && tar -xzf cpython-3.12.12.tar.gz"
    python_v12_install_cmd: str = "mkdir -p /openhands && cd /openhands && wget -q -O cpython-3.12.12.tar.gz https://github.com/astral-sh/python-build-standalone/releases/download/20251217/cpython-3.12.12+20251217-x86_64-unknown-linux-gnu-install_only.tar.gz && tar -xzf cpython-3.12.12.tar.gz"

    # Command to clone SWE-agent repository and install dependencies
    openhands_sdk_install_cmd_list: list[str] = [
        "/openhands/python/bin/pip install openhands-agent-server==1.6.0 openhands-sdk==1.6.0 openhands-tools==1.6.0 openhands-workspace==1.6.0 -i https://mirrors.aliyun.com/pypi/simple/",
        "git clone https://github.com/OpenHands/benchmarks.git /openhands/benchmarks",
        # "cd /openhands/benchmarks && git checkout c67349f4ce9bd5e72b394cfb5be91d8f33fe229c",
        "/openhands/python/bin/pip install datasets huggingface-hub jinja2 pandas Pillow toml tqdm openhands-sdk openhands-tools 'unidiff>=0.7.5,<0.8.0' 'modal>=1.1.4' swebench commit0 pytest-json-report -i https://mirrors.aliyun.com/pypi/simple",
    ]
...
class SweAgent(Agent):
    ...
    def __init__(self, sandbox: AbstractSandbox, config: OpenhandsConfig):
        """
