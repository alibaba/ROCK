...
class ModelServiceConfig(BaseModel):
    ...
    Attributes:
        ...
        anti_call_llm_request_log_path: Path to log anti-call LLM request payloads.
        anti_call_llm_response_log_path: Path to log anti-call LLM response outputs.
    """
    ...
    # TODO: 重命名为anti_call_llm_request_log_local_path
    anti_call_llm_request_log_path: str | None = None
    anti_call_llm_response_log_path: str | None = None
...
class ModelService:
    ...
    def __init__(self, sandbox: Sandbox, config: ModelServiceConfig):
        ...
        self._init_anti_call_llm_logs()

    def _init_anti_call_llm_logs(self) -> None:
        """Initialize anti-call LLM debug log files.

        Creates parent directories if they don't exist and initializes the log files.
        """
        if self.config.anti_call_llm_request_log_path:
            request_log_path = Path(self.config.anti_call_llm_request_log_path)
            request_log_path.parent.mkdir(parents=True, exist_ok=True)
            request_log_path.touch(exist_ok=True)
            logger.debug(f"Anti-call LLM request log initialized: {request_log_path}")

        if self.config.anti_call_llm_response_log_path:
            response_log_path = Path(self.config.anti_call_llm_response_log_path)
            response_log_path.parent.mkdir(parents=True, exist_ok=True)
            response_log_path.touch(exist_ok=True)
            logger.debug(f"Anti-call LLM response log initialized: {response_log_path}")
    ...
    def _write_request_log(self, index: int, payload: str | None) -> None:
        ...
        """
        if not self.config.anti_call_llm_request_log_path:
            return

        try:
            formatted_payload = self._format_json_content(payload)
            with open(self.config.anti_call_llm_request_log_path, "a", encoding="utf-8") as f:
                f.write(f"{'=' * 80}\n")
    ...
    def _write_response_log(self, index: int, output: str) -> None:
        ...
        """
        if not self.config.anti_call_llm_response_log_path:
            return

        try:
            formatted_output = self._format_json_content(output)
            with open(self.config.anti_call_llm_response_log_path, "a", encoding="utf-8") as f:
                f.write(f"{'=' * 80}\n")
    ...
    async def start(self) -> None:
        ...
        try:
            logger.info(f"[{sandbox_id}] Starting model service")
            ...
            logger.debug(f"[{sandbox_id}] Start command: {bash_start_cmd}")
    ...
    async def watch_agent(self, pid: str) -> None:
        ...
        try:
            ...
            logger.debug(f"[{sandbox_id}] Watch agent with pid={pid}, cmd: {bash_watch_cmd}")
    ...
    async def anti_call_llm(
        ...
        try:
            ...
            self._write_request_log(index, response_payload)
            ...
            self._write_response_log(index, result.output)
