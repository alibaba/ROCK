...
class SingleTaskPlanner:
    ...
    def __init__(self, *, preserve_job_name: bool = False):
    ...
    def plan(
        ...
        *,
        ...
        job_name = self._job_name(cloned, task, resolved_job_id)

        cloned.job_name = job_name
        cloned.labels["rock_job_id"] = str(resolved_job_id)
        ...
        if isinstance(cloned, HarborJobConfig):
            ...
            self._apply_bash_task(cloned, task, job_name, resolved_job_id)
        else:
    ...
    def _job_name(self, config: JobConfig, task: ResolvedTask, job_id: UUID) -> str:
        ...
        return f"{dataset_short}_{task_short}_{job_id.hex[:8]}"
    ...
    def _apply_harbor_task(config: HarborJobConfig, task: ResolvedTask) -> None:
        ...
        if isinstance(dataset_config, RegistryDatasetConfig):
            if task.dataset:
                dataset_config.name = (
                    task.dataset if "/" in task.dataset or not task.org else f"{task.org}/{task.dataset}"
                )
