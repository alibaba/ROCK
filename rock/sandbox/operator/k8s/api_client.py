...
class K8sApiClient:
    ...
    Centralizes all K8s API Server access and provides:
    - Rate limiting (configurable QPS and burst)
    - Local cache with watch-based sync (Informer pattern)
    - Consistent error handling
    - Simple CRUD interface for provider
    """
    ...
    def __init__(
        ...
        Args:
            ...
            burst: Maximum burst size (default: 10 for small clusters)
    ...
    async def start(self):
        ...
        # Start watch task for cache sync
    ...
    async def _watch_resources(self):
        ...
        while True:
            try:
                ...
                # Re-sync cache to avoid event loss
                try:
                    resource_version = await self._list_and_sync_cache()
