...
class BatchSandboxProvider(K8sProvider):
    ...
    async def get_status(self, sandbox_id: str) -> SandboxInfo:
        ...
        # Get basic sandbox info: host_ip, port_mapping, image
        host_ip, port_mapping, image = await self._fetch_k8s_resource(sandbox_id)
    ...
    async def stop(self, sandbox_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Deleted BatchSandbox: {sandbox_id} from namespace: {namespace}")
            return True
    ...
    async def _ensure_initialized(self):
        ...
        try:
            ...
            logger.info("Initialized K8s provider")
        except Exception as e:
    ...
    async def _fetch_k8s_resource(self, sandbox_id: str) -> tuple[str, dict[int, int], str]:
        """Fetch BatchSandbox resource and extract essential info from annotations.
        
        Args:
            sandbox_id: ID of the sandbox
            
        Returns:
            tuple: (host_ip, port_mapping, image)
            - host_ip: Pod IP from endpoints annotation (empty string if not allocated)
            - port_mapping: Port configuration from annotations
            - image: Container image from annotations
        ...
        try:
            ...
            image = annotations.get(K8sConstants.ANNOTATION_IMAGE, "")
            ...
            return host_ip, port_mapping, image
            
        except client.exceptions.ApiException as e:
            if e.status == 404:
                raise Exception(f"Sandbox {sandbox_id} not found")
            logger.error(f"Failed to fetch K8S resource for {sandbox_id}: {e}", exc_info=True)
            raise
        except Exception as e:
            logger.error(f"Unexpected error fetching K8S resource for {sandbox_id}: {e}", exc_info=True)
            raise
