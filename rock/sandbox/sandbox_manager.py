...
class SandboxManager(BaseManager):
    ...
    def __init__(
        ...
        self._test_proxy = SandboxProxyService(rock_config=rock_config, redis_provider=redis_provider)
    ...
    async def create_session(self, request: CreateSessionRequest) -> CreateBashSessionResponse:
        return await self._test_proxy.create_session(request)

    @monitor_sandbox_operation()
    async def run_in_session(self, action: Action) -> BashObservation:
        return await self._test_proxy.run_in_session(action)

    async def close_session(self, request: CloseBashSessionRequest) -> CloseBashSessionResponse:
        return await self._test_proxy.close_session(request)

    async def execute(self, command: Command) -> CommandResponse:
        return await self._test_proxy.execute(command)

    async def read_file(self, request: ReadFileRequest) -> ReadFileResponse:
        return await self._test_proxy.read_file(request)

    @monitor_sandbox_operation()
    async def write_file(self, request: WriteFileRequest) -> WriteFileResponse:
        return await self._test_proxy.write_file(request)

    @monitor_sandbox_operation()
    async def upload(self, file: UploadFile, target_path: str, sandbox_id: str) -> UploadResponse:
        return await self._test_proxy.upload(file, target_path, sandbox_id)
