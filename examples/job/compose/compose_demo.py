...
using ``ComposeJobConfig`` v2 — container orchestration is fully delegated to
a standard ``docker-compose.yaml`` file.

Layout inside the DinD sandbox after uploads:
    /rock/compose/docker-compose.yaml  ← uploaded from compose_file
    /rock/compose/main.sh              ← harbor runner entry-point
    /rock/compose/sidecars/            ← sidecar scripts
    /rock/runner.sh                    ← generated by ComposeTrial (minimal)
