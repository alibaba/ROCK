...
def main():
    ...
    workers = _resolve_workers(args.role, args.workers)
    uvicorn.run(
...
def _resolve_workers(role: str, override: int | None) -> int:
    if role != "proxy":
        return 1                           # admin 恒单进程(scheduler/Ray 单例)
    if override and override > 0:
        return override
    n = env_vars.ROCK_PROXY_WORKERS        # 默认 0 = auto
...
- `env_vars.py` 新增懒加载默认:`ROCK_PROXY_WORKERS`(默认 `0` = auto = `os.cpu_count()`)。
- worker 数最终建议 `min(cpu_count, 可用内存/单进程RSS)`(见容量预算),不要盲目等于 cpu_count。
...
- **worker 进程内存**:每个 worker 是完整 Python 进程(spawn 下几乎不共享),粗估单进程 RSS 100–300MB。`N worker ≈ N × 单进程RSS`,通常是多 worker 的**主要内存成本**,决定 worker 上限:`workers = min(cpu_count, 可用内存 / 单进程RSS)`。
...
2. **`_resolve_workers(role, override)`**:proxy 走 cpu_count/override/env;admin 恒 1。
