...
- IT-FC-05: FC function adapter (fc_rocklet/adapter) - import and runtime tests
- IT-FC-06: FC function adapter session operations
...
# ============================================================
# IT-FC-05: FC function adapter (fc_rocklet)
# ============================================================
...
class TestFCFunctionAdapter:
    ...
    def test_adapter_import(self):
        """IT-FC-05a: Verify adapter module can be imported."""
        from rock.deployments.fc_rocklet.adapter import server
    ...
    def test_adapter_handler_function(self):
        """IT-FC-05b: Verify WSGI handler exists."""
        from rock.deployments.fc_rocklet.adapter.server import handler

        assert callable(handler)

    def test_adapter_health_check(self):
        """IT-FC-05c: Verify health_check function works."""
        try:
            import gem  # noqa: F401
        except ImportError:
            pytest.skip("gem module not available")

        from rock.deployments.fc_rocklet.adapter.server import health_check
    ...
    def test_adapter_local_sandbox_runtime_available(self):
        """IT-FC-05d: Verify LocalSandboxRuntime is available."""
        try:
    ...
    def test_adapter_runtime_initialization(self):
        """IT-FC-05e: Verify runtime lazy initialization works."""
        try:
            import gem  # noqa: F401
        except ImportError:
            pytest.skip("gem module not available")

        from rock.deployments.fc_rocklet.adapter.server import _get_runtime

        # Reset runtime
        import rock.deployments.fc_rocklet.adapter.server as server_module
...
class TestFCFunctionAdapterSession:
    ...
    def test_route_request_unknown_path(self):
        """IT-FC-06a: Verify unknown path returns error."""
...
# ============================================================
# IT-FC-06-2: FC function adapter file operations
# ============================================================
...
class TestSandboxManagerFCIntegration:
    ...
    @pytest.mark.asyncio
    async def test_is_fc_sandbox_by_id(self):
        """IT-FC-07a: Verify FC sandbox detection checks deployment dict."""
        ...
        # Initially no FC sandboxes
        assert manager._is_fc_sandbox("fc-abc123") is False

        # Add a mock FC deployment
        from unittest.mock import MagicMock
        mock_deployment = MagicMock()
        manager._fc_deployments["fc-test-id"] = mock_deployment

        # Now should detect as FC sandbox
        assert manager._is_fc_sandbox("fc-test-id") is True
        assert manager._is_fc_sandbox("other-id") is False

    @pytest.mark.asyncio
    async def test_fc_deployments_dict_initialized(self):
        """IT-FC-07b: Verify FC deployments dict is initialized."""
        from rock.sandbox.sandbox_manager import SandboxManager
        from rock.config import RockConfig
        ...
        assert hasattr(manager, "_fc_deployments")
        assert isinstance(manager._fc_deployments, dict)
