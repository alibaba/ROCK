...
- IT-RUNTIME-05: Session affinity consistency across all runtimes
...
@pytest.fixture
def adapter_config(adapter_dir) -> dict:
    """Load adapter s.yaml configuration."""
    config_path = adapter_dir / "s.yaml"
    with open(config_path, "r") as f:
        return yaml.safe_load(f)
...
class TestSessionAffinityConsistency:
    ...
    def test_all_runtimes_have_session_affinity(
        self, runtime_config, container_config, adapter_config
    ):
        """IT-RUNTIME-05a: Verify all runtimes have session affinity config."""
        for config in [runtime_config, container_config, adapter_config]:
