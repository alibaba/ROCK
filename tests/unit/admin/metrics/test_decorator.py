...
def test_extract_sandbox_id_with_custom_extractor():
    ...
    result = _extract_sandbox_id(
        ...
        extract_sandbox_id=custom_extractor
    )
...
def test_extract_sandbox_id_with_param():
    result = _extract_sandbox_id(
        args=("arg1", "arg2"), 
        kwargs={"sandbox_id": "param-sandbox-id"}, 
        sandbox_id_param="sandbox_id"
    )
    assert result == "param-sandbox-id"

def test_extract_sandbox_id_with_position():
    result = _extract_sandbox_id(
        args=("arg1", "position-sandbox-id", "arg3"), 
        kwargs={}, 
        sandbox_id_position=2
    )
...
def test_get_user_info_success(mock_alive_key):
    ...
    async def async_mock_return_value(*args, **kwargs):
        return [{"user_id": "user123", "experiment_id": "exp456"}]
    ...
    user_id, experiment_id = asyncio.run(_get_user_info(mock_redis_provider, "test-sandbox"))
...
def test_build_attributes():
    ...
    attributes = _build_attributes("test_operation", "test-sandbox", mock_func, "user123", "exp456")
...
def test_record_metrics_success():
    ...
    mock_metrics_monitor.record_counter_by_name.assert_any_call(
        "test.success", 1, attributes
    )
    
    # Verify gauge and total counter were called
    assert mock_metrics_monitor.record_gauge_by_name.called
    mock_metrics_monitor.record_counter_by_name.assert_any_call(
        "test.total", 1, attributes
    )
...
def test_record_metrics_failure():
    ...
    mock_metrics_monitor.record_counter_by_name.assert_called_with(
        "test.failure", 1, error_attrs
    )
