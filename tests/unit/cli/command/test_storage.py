...
class TestStorageGet:
    ...
    async def test_generic_oss_exception_prints_failed(self, capsys):
        ...
        bucket_mock = MagicMock()
        bucket_mock.get_object_to_file = MagicMock(side_effect=RuntimeError("network exploded"))
...
class TestBuildGetTokenUrl:
    def test_appends_api_prefix_when_base_url_is_bare_host(self):
        # User passes just `https://admin/` — CLI auto-appends the FastAPI prefix
        # so they don't have to remember the routing detail.
        url = StorageCommand._build_get_token_url("https://admin.local")
        assert url == "https://admin.local/apis/envs/sandbox/v1/get_token?account=primary"

    def test_does_not_double_append_when_base_url_already_has_prefix(self):
        url = StorageCommand._build_get_token_url("https://admin.local/apis/envs/sandbox/v1")
        assert url == "https://admin.local/apis/envs/sandbox/v1/get_token?account=primary"
...
class TestHeaders:
    def test_auth_token_lifts_to_xrl_authorization_header(self):
        args = argparse.Namespace(auth_token="tok-2", extra_headers={"cluster": "c1"})
        headers = StorageCommand._build_headers(args)
        assert headers["xrl-authorization"] == "tok-2"
        assert headers["cluster"] == "c1"

    def test_no_auth_token_keeps_extra_headers_only(self):
        args = argparse.Namespace(auth_token=None, extra_headers={"cluster": "c1"})
        headers = StorageCommand._build_headers(args)
